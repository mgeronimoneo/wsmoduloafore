		...
		if (shRet == OK__)
		{
			...
			if (iTipoRetirox == CVE_RET_DESEMPLEO_IMSS)
				snprintf(cSql, sizeof(cSql), "SELECT limit 1 nss, salario_base_a, salario_base_b,nvl(to_char(fecha_fall_mat_des, '%cY-%cm-%cd'), '1900-01-01'), nvl(to_char(fecha_ini_vigencia, '%cY-%cm-%cd'), '1900-01-01'), 0::double precision, 0::double precision FROM ret_parcial_resol "
											 "WHERE nss = '%s' AND tipo_prestacion = 6  AND salario_base_a is not null AND salario_base_a is not null ORDER BY fecha_fin_vigencia DESC;",
						 37, 37, 37, 37, 37, 37, cNssx);
			else
				snprintf(cSql, sizeof(cSql), "SELECT limit 1 nss, salario_base_a, salario_base_b, nvl(to_char(fecha_fall_mat_des, '%cY-%cm-%cd'), '1900-01-01'), nvl(to_char(fecha_ini_vigencia, '%cY-%cm-%cd'), '1900-01-01'), 0::double precision, 0::double precision FROM ret_parcial_resol "
											 "WHERE nss = '%s' AND tipo_prestacion = 7  AND salario_base_a is not null AND salario_base_a is not null ORDER BY fecha_fin_vigencia DESC;",
						 37, 37, 37, 37, 37, 37, cNssx);
