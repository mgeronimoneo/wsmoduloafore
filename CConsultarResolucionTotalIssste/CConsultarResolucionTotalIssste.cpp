...
short CConsultarResolucionTotalIssste::consultaDatamart(char *cCurp, char *cRetiro)
{
	...
	int iPuerto=0;
	sprintf(cTexto, "[%s::%s]=================IniciA=================[%s]", __FILE_MIN__, __FUNCTION__, cSql);
	...
	{	
		...
		if(iFlagReplica == 0)
		{
			...
			if(xSelIssstep.Exec(cSql))
			{
				...
				if(xSelIssstep.leer())
				{
					...
					sprintf(stDMI.cNss				,"%s",xSelIssstep.cNss				);
					sprintf(stDMI.cNssissste		,"%s",xSelIssstep.cNssissste			);
					sprintf(stDMI.cCurp				,"%s",xSelIssstep.cCurp				);	
					sprintf(stDMI.cSecpension		,"%s",xSelIssstep.cSecpension		);
					sprintf(stDMI.cNombreDatamart	,"%s",xSelIssstep.cNombreDatamart	);
					sprintf(stDMI.cPaternoDatamart	,"%s",xSelIssstep.cPaternoDatamart	);
					sprintf(stDMI.cMaternoDatamart	,"%s",xSelIssstep.cMaternoDatamart	);			
					sprintf(stDMI.cNumconcesion		,"%s",xSelIssstep.cNumconcesion		);
					sprintf(stDMI.cTiporetiro		,"%s",xSelIssstep.cTiporetiro		);
					sprintf(stDMI.cRegimen			,"%s",xSelIssstep.cRegimen			);
					sprintf(stDMI.cTiposeguro		,"%s",xSelIssstep.cTiposeguro		);
					sprintf(stDMI.cTipopension		,"%s",xSelIssstep.cTipopension		);
					sprintf(stDMI.cCvepension	  	,"%s",xSelIssstep.cCvepension		);
					sprintf(stDMI.cFechainiciopen 	,"%s",xSelIssstep.cFechainiciopen	);
					sprintf(stDMI.cFecharesolucion	,"%s",xSelIssstep.cFecharesolucion	);				
			...
			else
			{
				...
				sprintf( cTexto, "[%s] Err: %s", __FUNCTION__, xSelIssstep.odbc->LastErrStr() );
				CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
			}
		}
		else
		{
			...
			if(shRet == MSJ_OK__)
			{		
				//sprintf(cSql, "EXECUTE FUNCTION fn_obtenerdatosdatamartissste('CURC531120HDFRVR08','B');");	
				...
				sprintf(cTexto, "[%s::%s] Consulta DataMart : [%s]", __FILE_MIN__, __FUNCTION__, cSql);		
				CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
				...
				{				
					...
					{	
						...
						sprintf(stDMI.cNss				,"%s",xSelIssste.cNss				);
						sprintf(stDMI.cNssissste		,"%s",xSelIssste.cNssissste			);
						sprintf(stDMI.cCurp				,"%s",xSelIssste.cCurp				);	
						sprintf(stDMI.cSecpension		,"%s",xSelIssste.cSecpension		);
						sprintf(stDMI.cNombreDatamart	,"%s",xSelIssste.cNombreDatamart	);
						sprintf(stDMI.cPaternoDatamart	,"%s",xSelIssste.cPaternoDatamart	);
						sprintf(stDMI.cMaternoDatamart	,"%s",xSelIssste.cMaternoDatamart	);			
						sprintf(stDMI.cNumconcesion		,"%s",xSelIssste.cNumconcesion		);
						sprintf(stDMI.cTiporetiro		,"%s",xSelIssste.cTiporetiro		);
						sprintf(stDMI.cRegimen			,"%s",xSelIssste.cRegimen			);
						sprintf(stDMI.cTiposeguro		,"%s",xSelIssste.cTiposeguro		);
						sprintf(stDMI.cTipopension		,"%s",xSelIssste.cTipopension		);
						sprintf(stDMI.cCvepension	  	,"%s",xSelIssste.cCvepension		);
						sprintf(stDMI.cFechainiciopen 	,"%s",xSelIssste.cFechainiciopen	);
						sprintf(stDMI.cFecharesolucion	,"%s",xSelIssste.cFecharesolucion	);				
				...
				{			
					...
					sprintf( cTexto, "[%s] Err: [%s][%i]", __FUNCTION__, xSelIssste.odbc->LastErrStr(),shRet );
					CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
