...
short CConsultaSaldoVivienda::consultavivienda(char *cNss, char *fecha_solicitud, char *fecha_ini_pen)
{
	...
	{	
		...
		sprintf(cSql, "SELECT fnvalidacionreplicas FROM fnvalidacionreplicas(1, 2);");
		printf("%s\n",cSql);
		...
		if(iFlagReplica == 0)
		{
			...
			sprintf(fecha_ini_pen, "%s", fechainipens);
			sprintf(fecha_solicitud, "%s", fechasolicitud);
			...
			sprintf(cSql, "SELECT nss,dmontoaccviv,dmontopesviv FROM fnobtenersaldovivret_e('%s', '%s', '%s');",cNss,fecha_solicitud,fecha_ini_pen);
			...
			if(xSelVivienda.Exec(cSql))
			{
				...
				if(xSelVivienda.leer())
				{
					...
					sprintf(stDMI.sNss				,"%s",xSelVivienda.cNss				);
					// printf("%s  postgresql\n",stDMI.sNss	);
					sprintf(stDMI.dMontoaccviv		,"%s",xSelVivienda.dMontoaccviv	);
						// printf("%s  postgresql\n",stDMI.dMontoaccviv);
					sprintf(stDMI.dMontopesviv				,"%s",xSelVivienda.dMontopesviv	);
						// printf("%s  postgresql\n",stDMI.dMontopesviv);	
			...
			else
			{
				...
				sprintf( cTexto, "[%s] Err: %s", __FUNCTION__, xSelVivienda.odbc->LastErrStr() );
				CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
			}
		}
		else
		{
			...
			{		
				...
				sprintf(cSql, "EXECUTE FUNCTION fn_afop_saldo_viv_ret_e('%s', '%s', '%s');",cNss,fecha_solicitud,fecha_ini_pen);	
				sprintf(cTexto, "[%s::%s] Consulta Vivienda : [%s]", __FILE_MIN__, __FUNCTION__, cSql);		
				CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
				printf("%s\n",cSql);
				...
				{			
					...
					shRet = ERR_EXEC_SQL;
					xSelVivienda.odbc->GetLastError( xSelVivienda.GetHstmt() );
					sprintf( cTexto, "[%s] Err: [%s][%i]", __FUNCTION__, xSelVivienda.odbc->LastErrStr(),shRet );
					CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
