...
short CAportaciones::ObtenerAportaciones(char* sNss, char* dFechaIniPension, int opcion)
{
	...
	sprintf(cTexto, "[%s] ------ Inicia [Nss: %s], [Inicio Pension: %s]------",  __FUNCTION__, sNss,dFechaIniPension);
	...
	if(shRet == OK__)
	{
		...
		sprintf(cTexto, "[%s] ------ Inicia ------",  __FUNCTION__);
		...
		if(shRet==OK__) 
		{
			...
			if(shRet==OK__)
			{
				...
				if(shRet==OK__)
				{
					...
					sprintf(auxFechaPension, "MDY(%s,%s,%s)",f1, f2, f3 );
					
					CFnAportaciones xSelAportacion(&odbcIfx);
					sprintf(cSql, "EXECUTE FUNCTION fn_obtener_aportaciones_posteriores_rt( '%s', %s, %i);", sNss, auxFechaPension, opcion);
					
					sprintf( cTexto, "[%s]Se Ejecuto: %s", __FUNCTION__,cSql);
					CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
					...
					if(xSelAportacion.Exec(cSql))
					{
						...
						{
							sprintf( cTexto, "[%s]Se Retorno la cantidad de [%lf]  aportaciones ", __FUNCTION__,xSelAportacion.iAportaciones);
							CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
					...
					else
					{
						...
						sprintf( cTexto, "[%s] Err: %s", __FUNCTION__, xSelAportacion.odbc->LastErrStr() );
						CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
					}
				}
				else
	...
	sprintf(cTexto, "[%s] Retorno con caso %i.",  __FUNCTION__, shRet);
	CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
	sprintf(cTexto, "[%s] ------ Termina [Nss: %s], [Inicio Pension: %s]------",  __FUNCTION__, sNss,dFechaIniPension);
	CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
