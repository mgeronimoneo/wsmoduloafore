...
{ register int err;
...
#else
    if (soap_valid_socket(soap->master) || soap_valid_socket(soap->socket) || true ) /* se agrega "|| true" para forzar OK, requerido por xinetd  .:: Eduardo Osuna 08/MAR/13:. */ /* standalone application (socket) or CGI (stdin/out)? */
#endif
  ...
  { const char *s = *soap_faultcode(soap);
    ...
    if (soap->rpmreqid || soap_valid_socket(soap->master) || soap_valid_socket(soap->socket)) /* RPM behaves as if standalone */
#else
    if (soap_valid_socket(soap->master) || soap_valid_socket(soap->socket) || true ) /* se agrega "|| true" para forzar OK, requerido por xinetd  .:: Eduardo Osuna 13/AGO/13:. *//* standalone application */
#endif
    { sprintf(soap->tmpbuf, "HTTP/%s %s", soap->http_version, s);
      if ((err = soap->fposthdr(soap, soap->tmpbuf, NULL)))
        return err;
