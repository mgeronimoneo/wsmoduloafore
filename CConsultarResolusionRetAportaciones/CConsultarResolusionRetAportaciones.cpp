...
short CConsultarResolusionRetAportaciones::ConsultarResolusion(char *cCurpx, char *cNssx)
{
	...
	sprintf(cTexto, "[%s] ------ Inicia [Curp: %s Nss: %s]------",  __FUNCTION__, cCurpx, cNssx);
	...
	if(shRet == OK__)
	{
		...
		if(shRet == OK__)
		{
			...
			if( iFlagReplica == 0 )
			{
				...
				sprintf(cSql, "SELECT fnconsultaresolpensionretaportaciones AS suma FROM fnconsultaresolpensionretaportaciones('%s','%s');", cCurpx, cNssx);
				CUtileriasAfo::grabarLogx(cRutaLog, cSql);
				...
				if(xSelResol.Exec(cSql))
				{
					...
					{	
						...
						sprintf(cTexto, "No hay Registros en la BD de SERVICIOSAFORE CURP-> %s NSS-> %s", cCurpx, cNssx);
						CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
			...
			{
				sprintf(cTexto, "No se a replicado a la BD de SERVICIOSAFORE CURP-> %s NSS-> %s", cCurpx, cNssx);
				CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
				shRet=this->ConsultarResolusionBD(cCurpx, cNssx);
			}
		}
	...
	sprintf(cTexto, "[%s] ------ Termina [Curp: %s Nss: %s]------",  __FUNCTION__, cCurpx, cNssx);
	CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
	return shRet;
}
short CConsultarResolusionRetAportaciones::ConsultarResolusionBD(char *cCurpx, char *cNssx)
{
	...
	sprintf(cTexto, "[%s] validar registro en BD CURP: %s NSS: %s",  __FUNCTION__, cCurpx, cNssx);
	CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
	...
	if(shRet==OK__)
	{
		...
		if(shRet==OK__)
		{
			...
			sprintf(cSql, "EXECUTE FUNCTION fn_consulta_resolpension_retaportaciones('%s','%s');", cCurpx, cNssx);
			CUtileriasAfo::grabarLogx(cRutaLog, cSql);
			...
			if(xSelResol.Exec(cSql))
			{
				...
				if(xSelResol.leer())
				{
					...
					sprintf(cTipoPension, "%s", xSelResol.tTipo_Pension);
					shRet=CConsultarResolusionRetAportaciones::obtenerTipoSolicitante(iTipoSeguro,cTipoPension);
					//shRet=OK__;
				}
				else
				{	
					sprintf(cTexto, "No hay Registros en la BD de SAFRE CURP-> %s NSS-> %s", cCurpx, cNssx);
					CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
					iResolusion=0;
					shRet=ERR_NO_HAY_REG_BD;
				}
...
short CConsultarResolusionRetAportaciones::obtenerTipoSolicitante(int iTipoSeguro, char *cTipoPension)
{
	short shRet=DEFAULT__;
	C_SumaRegs xSelResol(&odbcPg);
	sprintf(cSql, "SELECT fnobtenertiposolicitantextipopension AS suma FROM fnobtenertiposolicitantextipopension(%i,'%s');", iTipoSeguro, cTipoPension);
	if(xSelResol.Exec(cSql))
	{
		xSelResol.activarCols();
		if(xSelResol.leer())
		...
		{	
			sprintf(cTexto, "No hay Registros en la BD de SERVICIOSAFORE TipoSeguro-> %s TipoPension-> %s", iTipoSeguro, cTipoPension);
			CUtileriasAfo::grabarLogx(cRutaLog, cTexto);
			iResolusion=0;
			shRet=ERR_NO_HAY_REG_BD;
		}
